class Controller(object):
    ...
    def show_note_date(self, date):
        try:
            note = self.model.read_note_date(date)
            self.view.show_note(note)
        except:
            self.view.display_note_date_not_exist(date)
    ...
    def note_date_exist(self, search_date):
        notes = self.model.read_notes()
        for note in notes:
            if search_date in note.date:
                return True
        else:
            self.view.display_note_date_not_exist(search_date)
            return False
